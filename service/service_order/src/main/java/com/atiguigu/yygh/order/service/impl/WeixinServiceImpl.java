...
import com.atiguigu.yygh.order.utils.ConstantPropertiesUtils;
import com.atiguigu.yygh.order.utils.HttpClient;
...
public class WeixinServiceImpl implements WeixinService {
    ...
    public Map createNative(Long orderId) {
        try {
            ...
            //3、设置参数 调用微信生成二维码接口
            //把参数转为xml格式，使用商户key进行加密
            Map paramMap = new HashMap();
            paramMap.put("appid", ConstantPropertiesUtils.APPID); //商户id
            paramMap.put("mch_id", ConstantPropertiesUtils.PARTNER);
            paramMap.put("nonce_str", WXPayUtil.generateNonceStr());//唯一随机字符串
            String body = order.getReserveDate() + "就诊" + order.getDepname();
            paramMap.put("body", body);//就诊人
            paramMap.put("out_trade_no", order.getOutTradeNo());
            //paramMap.put("total_fee", order.getAmount().multiply(new BigDecimal("100")).longValue()+"");
            paramMap.put("total_fee", "1");//为了测试用
    ...
    public Map<String, String> queryPayStatus(Long orderId) {
        try{
            ...
            client.setXmlParam(WXPayUtil.generateSignedXml(paramMap, ConstantPropertiesUtils.PARTNERKEY));//第二个参数是加密
            client.setHttps(true);
            client.post();//提交
